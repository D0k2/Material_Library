...
class PropertyComparisonTab(ttk.Frame):
    ...
    def _setup_widgets(self):
        ...
        # Убедитесь, что ALL_PROPERTIES_MAP доступен в этом скоупе
...
1.2. Укажите папку, в которой находятся ваши JSON-файлы материалов и файл `application_area.txt`.
1.3. Приложение загрузит все найденные материалы.
...
2.1. Подбор по температуре:
   - Выберите "Область применения" для фильтрации материалов. По умолчанию показываются все.
   - Введите расчетную температуру в поле "Температура, °С".
   - Нажмите "Рассчитать". В таблице отобразятся значения свойств для каждого материала при заданной температуре.
   - Если точного значения для температуры нет, оно будет вычислено линейной интерполяцией. Если значение вне диапазона или отсутствует, будет показан "-".
   - Для сортировки по свойству кликните на заголовок соответствующего столбца.
   - Правый клик на ячейке позволяет скопировать ее значение.

2.2. Сравнение материалов (свойства):
   - Выберите свойство из выпадающего списка.
   - В списке материалов выберите один или несколько (с зажатой клавишей Ctrl или Shift).
   - Нажмите "Построить график". На графике отобразится зависимость выбранного свойства от температуры для каждого материала.

2.3. Сравнение материалов (хим. состав):
   - Выберите один или несколько материалов для сравнения.
   - В таблице отобразится их химический состав по разным источникам (если их несколько).
   - В верхней области отобразаятся все элементы которые содержаться в выбранных материалов, в окошки можно вводить значения для подбора материала с заданным хим. составом
   - При наведении мышью на элемент в таблице всплывает подсказка как данный элемент влияет на свойства материала

3. ВКЛАДКА "ДОБАВЛЕНИЕ / РЕДАКТИРОВАНИЕ МАТЕРИАЛА"

Эта вкладка позволяет создавать новые материалы или изменять существующие.

3.1. Редактирование существующего материала:
   - Выберите материал из выпадающего списка вверху вкладки.
   - Поля во всех подвкладках ("Общие данные", "Физические свойства" и т.д.) заполнятся данными выбранного материала.
   - Внесите изменения. Для таблиц (например, "Температура-Значение") двойной клик на ячейке активирует режим редактирования.
   - Для сохранения изменений используйте "Файл" -> "Сохранить".
   - Если вы переключитесь на другой материал без сохранения, ваши изменения будут отменены.

3.2. Создание нового материала:
   - Нажмите кнопку "Создать новый".
   - Все поля очистятся, и вы сможете заполнить их с нуля.
...
4.1. Структура JSON: Приложение строго следует заданной структуре JSON. Редактирование файлов вручную с нарушением структуры может привести к ошибкам при загрузке.
4.2. Файл `application_area.txt`: Этот файл должен находиться в той же директории, что и JSON-файлы. Он содержит список областей применения, по одной на строку. Этот список используется в выпадающих меню и в редакторе.
4.3. Сохранение: Команды меню "Сохранить" и "Сохранить как..." активны только после того, как в редакторе выбран материал для изменения или создан новый.
"""
